...
def test_provider_url_canonicalization() -> None:
    assert (
        canonical_url("https://jobs.smartrecruiters.com/Acme/123-role?ref=x", "smartrecruiters")
        == "https://jobs.smartrecruiters.com/Acme/123-role"
    )
    assert (
        canonical_url("https://apply.workable.com/acme/j/ABC123/?source=x", "workable")
...
def test_merge_prefers_real_vps_outcome() -> None:
    ...
    incoming = [
        {"job_url": "https://apply.workable.com/acme/j/ABC123/", "status": "CAPTCHA_REQUIRED"}
    ]
