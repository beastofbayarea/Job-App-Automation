...
def test_continuous_ashby_entrypoint() -> None:
    with patch("job_application_automation.core.continuous_ashby._continuous_main", return_value=0) as mock_main:
...
def test_continuous_greenhouse_entrypoint() -> None:
    with patch("job_application_automation.core.continuous_greenhouse._continuous_main", return_value=0) as mock_main:
...
def test_continuous_lever_entrypoint() -> None:
    with patch("job_application_automation.core.continuous_lever.run_continuous_worker", return_value=0) as mock_main:
