"""Expanded unit and mock integration tests for breezy.py ATS engine."""

from pathlib import Path
from unittest.mock import MagicMock, patch
import pytest

from job_application_automation.engines.breezy import (
    _parser,
    main,
    run,
)
...
def test_breezy_parser_and_main_help() -> None:
    parser = _parser()
...
def test_breezy_run_mocked(tmp_path: Path) -> None:
    ...
    with patch("job_application_automation.engines.breezy.sync_playwright", return_value=MagicMock(__enter__=MagicMock(return_value=mock_playwright))), \
         ...
         patch("job_application_automation.engines.breezy.capture_screenshot", return_value="screenshot.png"):

        res = run(
            url="https://example.breezy.hr/p/123/apply",
            resume=resume_file,
            config=config,
            company="Example",
            role="AI Engineer",
