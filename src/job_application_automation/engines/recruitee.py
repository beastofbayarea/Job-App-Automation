"""Recruitee ATS application engine built on the shared engine foundation."""
    ...
    with sync_playwright() as playwright:
        ...
        try:
            try:
                ...
                # Recruitee is a React SPA — wait for dynamic content to render
                page.wait_for_timeout(2000)
            except Exception:
                pass

            apply_btn = first_visible(
                page.locator('a:has-text("Apply for this job"), button:has-text("Apply now"), a:has-text("Apply"), a[href*="apply"]')
            )
            if apply_btn:
                try:
                    apply_btn.click()
            ...
            filled_fn = fill_first(page, 'input[name="candidate[first_name]"], input[name="first_name"]', first_name)
            filled_ln = fill_first(page, 'input[name="candidate[last_name]"], input[name="last_name"]', last_name)
            if not filled_fn or not filled_ln:
                fill_first(page, 'input[name="candidate[name]"], input[id*="name"], input[name="name"]', full_name)

            fill_first(page, 'input[name="candidate[email]"], input[name="email"], input[type="email"]', email)
            if phone:
                fill_first(page, 'input[name="candidate[phone]"], input[name="phone"], input[type="tel"]', phone)
            if headline:
                fill_first(page, 'input[name*="headline"], input[name*="title"]', headline)
            if linkedin:
            ...
            screenshot = capture_screenshot(page, screenshot_dir, company or "Recruitee", "prefill")
            ...
            if live_submit:
                if captcha:
                    ...
                    if submit_btn:
                        ...
                        submitted = True
                        screenshot = capture_screenshot(page, screenshot_dir, company or "Recruitee", "submitted")
...
def _parser() -> argparse.ArgumentParser:
    return build_engine_parser("Recruitee application engine")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        logger.exception("Recruitee engine failed")
        result = {
            "success": False,
