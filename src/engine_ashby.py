...
DEFAULT_CONFIG = {
    "candidate": {
        ...
        "fallback_email": "", "location": "", "city": "", "state": "", "country": "",
        "zip_code": "", "nationality": "", "citizenship": "", "linkedin": "",
        "portfolio": "", "website": "", "twitter": "", "researchgate": "",
        "sciencedirect": "", "street_address": "", "address_2": "", "gender": "",
        "pronouns": "", "race": "", "age": "", "veteran": "", "transgender": "",
        "orientation": "", "disability": "", "communities": [], "screening_answers": {}
    },
...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"Config missing required candidate fields: {', '.join(missing)}")
    cfg.setdefault("defaults", {})
    cfg.setdefault("paths", {})
    cfg.setdefault("company_overrides", {})
    candidate = cfg["candidate"]
    offset = candidate.get("start_date_offset_days", 14)
    try:
...
def ss(page: Page, directory: Path, company: str, tag: str) -> str:
    ...
    try:
        ...
        try:
            page.screenshot(path=str(jpg_path), type="jpeg", quality=85, full_page=fp, animations="disabled", timeout=2500)
            logger.info("Screenshot [%s]: %s", tag, jpg_path.name)
            return str(jpg_path)
        except Exception as primary_exc:
            try:
                page.screenshot(path=str(jpg_path), type="jpeg", quality=85, full_page=False, animations="disabled", timeout=2000)
...
def select_ashby_combobox(
    ...
    try:
        ...
        no_results = page.get_by_text(
            re.compile(r"^\s*No results\s*$", re.I)
        )
        ...
        for candidate in candidates:
            options = page.locator(option_selector).filter(has_text=re.compile(re.escape(candidate), re.I))
...
def _configured_answer(
    ...
    for fragment, answer in answers.items():
        if str(fragment).strip().lower() in normalized_question and answer not in (None, ""):
    ...
    for group in yesno_groups:
        try:
            ...
            if configured.lower() not in ("yes", "true", "1", "no", "false", "0"):
                logger.warning("Skipping non-boolean configured answer for question: %s", q_lower[:120])
                continue
...
def _fill_radio_groups(page: Page, profile: Mapping[str, Any]) -> None:
    ...
    for name, radios in radio_groups.items():
        try:
            ...
            target_text = None
            if any(k in q_lower for k in ("level", "experience", "proficiency", "scale", "rating", "skill", "knowledge", "seniority", "ability", "competency")):
            ...
            selected = False
            targets = [target_text]
            if target_text.lower() in ("male", "man"):
                targets = ["Male", "Man"]
            elif target_text.lower() == "asian":
                targets = ["Asian", "Asian or Asian American"]
            elif target_text.lower() in ("indian", "india"):
                targets = ["Indian", "India"]
            elif "," in target_text:
                targets = [target_text, target_text.split(",")[0].strip()]
            elif target_text.lower() == "yes":
                targets = ["Yes"]
            ...
            if not selected:
                for radio in radios:
                    ...
                    if parent_opt and any(t.lower() in parent_opt.get('text', '').lower() for t in targets):
                        radio.check(force=True)
                        selected = True
                        break
...
def _refresh_selected_choice_groups(page: Page) -> None:
    ...
    for group in page.locator("div[class*='_yesno']").all():
        try:
            ...
            alternate = group.locator("button").filter(
                has_not_text=re.compile(rf"^\s*{re.escape(intended)}\s*$", re.I)
            ).first
            if alternate.count() and alternate.is_visible():
                alternate.click()
                human_delay(0.1, 0.2)
            target = group.locator("button").filter(
                has_text=re.compile(rf"^\s*{re.escape(intended)}\s*$", re.I)
            ).first
            target.click()
            human_delay(0.1, 0.2)
    ...
    for group in groups:
        try:
            ...
            for value in requested:
                label = group.locator("label").filter(
...
def _is_preferred_work_region(option_text: str) -> bool:
    ...
    region_terms = (
        # United States
        "united states", "remote us", "u.s.", "usa", "san francisco",
        "new york", "seattle", "raleigh", "austin", "boston", "chicago",
        "denver", "los angeles", "miami", "portland", "washington",
        # Europe
        "europe", "emea", "london", "amsterdam", "berlin", "paris",
        "dublin", "madrid", "munich", "zurich", "stockholm", "warsaw",
        # Asia
        "asia", "apac", "singapore", "tokyo", "seoul", "hong kong",
...
def fill_eeo(page: Page, profile: Mapping[str, Any]) -> None:
    ...
    def _sel(kw: str, val: str) -> None:
        ...
        groups = page.locator("fieldset, [role='group'], div[class*='field'], div[class*='Field']").filter(has_text=re.compile(kw, re.I))
        for i in range(min(groups.count(), 5)):
            grp = groups.nth(i)
            lab = grp.locator("label, [role='option'], button").filter(has_text=re.compile(regex_pat, re.I)).first
            if lab.count() and lab.is_visible():
                selected_input = lab.locator(
                ...
                if not selected_input.count():
                    ...
                    if input_id:
                        selected_input = page.locator(
                            f'[id="{input_id}"][type="radio"], '
                            f'[id="{input_id}"][type="checkbox"]'
                        ).first
    ...
    try:
        ...
        comm_groups = page.locator("fieldset, [role='group'], div[class*='field']").filter(has_text=re.compile(r"community|communities|identity|neurodiverse", re.I))
...
def fill_personal_and_files(
    ...
    if not resume_loc.count():
        resume_field = page.locator(
            ".ashby-application-form-field-entry"
        ).filter(has_text=re.compile(r"^\s*Resume\b", re.I)).first
        resume_loc = resume_field.locator('input[type="file"]').first
    if not resume_loc.count():
        for finp in page.locator('input[type="file"]').all():
            lbl = finp.evaluate("el => (el.closest('div')||el.parentElement||el).innerText||''").lower()
        ...
        try:
            ...
            if re.search(r"\.(pdf|docx?|odt|rtf)\b", existing_text, re.I):
                ...
                for _ in range(20):
                    time.sleep(0.5)
                    field_text = (
                        resume_field.inner_text()
                        if resume_field.count()
                        else ""
                    )
    ...
    fn_loc = page.locator('input[id*="first" i], input[name*="first" i], input[aria-label*="First Name" i]').first
    ln_loc = page.locator('input[id*="last" i], input[name*="last" i], input[aria-label*="Last Name" i]').first
    pref_loc = page.locator('input[id*="preferred" i], input[name*="preferred" i], input[aria-label*="Preferred" i]').first

    if not fn_loc.count() or not ln_loc.count():
        for inp in page.locator('input:visible').all():
            lbl = inp.evaluate("el => (el.labels?.[0]?.innerText || el.closest('div') || el.parentElement || el).innerText || ''").lower()
            if not fn_loc.count() and any(k in lbl for k in ("first name", "given name")) and "last" not in lbl:
                fn_loc = inp
            elif not ln_loc.count() and any(k in lbl for k in ("last name", "family name", "surname")) and "first" not in lbl:
                ln_loc = inp
            elif not pref_loc.count() and any(k in lbl for k in ("preferred name", "nickname", "goes by", "alias")):
                pref_loc = inp
    ...
    if not name_loc.count():
        for inp in page.locator('input:visible').all():
            ...
            if "name" in lbl and "company" not in lbl and "file" not in lbl and "first" not in lbl and "last" not in lbl and "preferred" not in lbl:
                name_loc = inp
                break
    if name_loc.count() and name_loc.is_visible():
        full = " ".join(part for part in (profile.get("first_name", ""), profile.get("last_name", "")) if part)
        if full:
    ...
    email_loc = page.locator('input[id="_systemfield_email"], input[name="_systemfield_email"], input[type="email"]').first
    ...
    phone_loc = page.locator('input[id="_systemfield_phone"], input[name*="phone" i], input[type="tel"]').first
    ...
    loc_input = page.locator('input[id="_systemfield_location"], input[name="_systemfield_location"], input[id*="location" i], input[name*="location" i], input[placeholder*="location" i], input[placeholder*="city" i], input[aria-label*="location" i]').first
    if not loc_input.count() or not loc_input.is_visible():
        ...
        if location_field.count():
            loc_input = location_field.locator(
    ...
    if not loc_input.count() or not loc_input.is_visible():
        for inp in page.locator('input:visible').all():
            ...
            if any(k in lbl for k in ("where are you currently located", "current location", "where are you located", "where are you based", "location")):
                loc_input = inp
                break
    ...
    li = page.locator('input[id*="linkedin" i], input[name*="linkedin" i], input[placeholder*="linkedin" i]').first
...
def fill_secondary(
    ...
    for inp in page.locator('input[placeholder*="date" i]:visible, input[placeholder*="pick" i]:visible, input[type="date"]:visible, input[id*="start" i]:visible, input[name*="start" i]:visible').all():
        try:
            ctx = inp.evaluate("""el => {
            ...
            if start_date_str and any(k in ctx for k in ("start", "available", "when can you", "start date", "date")):
    ...
    for inp in page.locator('input[placeholder*="Start typing"]:visible, input[placeholder*="type"]:visible, input[role="combobox"]:visible, input[name*="location" i]:visible, input[id*="location" i]:visible, input[placeholder*="location" i]:visible, input[placeholder*="city" i]:visible').all():
        try:
            ctx = inp.evaluate("""el => {
                const field = el.closest(
                    '.ashby-application-form-field-entry, fieldset, [role="group"]'
            ...
            if any(k in ctx for k in ("hear", "referral", "source", "how did you")):
                ...
                if not inp.input_value().strip():
                    ...
                    if location:
                        select_ashby_combobox(page, inp, location, fallback_value=profile.get("city"))
    ...
    for idx, ta in enumerate(essay_controls, 1):
        try:
            ...
            if any(t in lbl for t in ("hear", "source", "referral")):
                ...
                val = (
                    configured_essay or
                    (essay_dict.get("why") if any(w in lbl for w in ("why", "interest", "mission", "fit", "inspire")) else None) or
                    (essay_dict.get("product") if any(w in lbl for w in ("product", "build", "excellence", "signal", "achievement", "project", "experience")) else None) or
                    essay_dict.get(f"essay_{idx}") or
                    essay_dict.get("default") or
                    default_essay or
                    generate_essay_safely(lbl, jd_text, company, role)
                )
                if val:
    ...
    for inp in page.locator("input:visible").all():
        try:
            ...
            mapping = [
                ...
                (("nationality", "citizenship"), profile.get("nationality") or profile.get("citizenship")),
                (("country",), profile.get("country")),
                (("linkedin",), profile.get("linkedin")),
                (("other url", "other website", "website", "goodreads"), profile.get("website")),
                (("portfolio", "researchgate"), profile.get("portfolio") or profile.get("researchgate")),
                (("twitter", "x.com"), profile.get("twitter")),
                (("sciencedirect",), profile.get("sciencedirect")),
                (("street address", "address_line_1", "address line 1"), profile.get("street_address")),
                (("suite", "apt", "unit", "address line 2"), profile.get("address_2")),
                ((r"\bcity\b|\blocation\b",), profile.get("location") or profile.get("city")),
                ((r"\bstate\b|\bregion\b",), profile.get("state") if "statement" not in lbl else None),
            ...
            if not matched and not inp.input_value().strip():
                ...
                elif any(w in lbl for w in ("hear", "source", "referral", "how did")) and defaults.get("source"):
    ...
    for field in page.locator("fieldset, [role='group'], div[class*='field'], div[class*='Field'], div[class*='Question'], div[class*='_yesno']").all():
        try:
            ...
            configured = _configured_answer(profile, q_lower)
            exact_option = (
                field.locator("[role='radio'], button, label").filter(
                    has_text=re.compile(
...
def _locate_submit_btn(page: Page):
    ...
    if btn.count() and btn.is_visible():
        return btn
    btn = page.locator('button[type="submit"]:visible').filter(has_not=page.locator('button[class*="_option"], button[class*="_remove"], button[class*="_secondary"]')).first
    if btn.count() and btn.is_visible():
        return btn
...
def can_advance(page: Page) -> bool:
    if _shutdown:
        return False
    next_btn = page.locator('button:has-text("Next"):visible, button:has-text("Continue"):visible, button:has-text("Save and continue"):visible, button:has-text("Save & Continue"):visible').first
...
def _education_field_container(group: Any, label_pattern: str) -> Any:
    label = group.locator("label").filter(
        has_text=re.compile(rf"^\s*{label_pattern}\s*\*?\s*$", re.I)
    ).first
    if not label.count():
        return group.locator(
            ".ashby-application-form-field-entry"
        ).filter(has_text=re.compile(label_pattern, re.I)).first
...
def _choose_education_option(
    ...
    *,
    ...
    try:
        ...
        ).filter(has_text=re.compile(rf"^\s*{re.escape(value)}\s*$", re.I)).first
...
def fill_education_history(
    ...
    group = page.locator(
        '.ashby-application-form-field-entry'
        '[data-field-path="_systemfield_education_history"]'
    ).first
    if not group.count():
        group = page.locator(".ashby-application-form-field-entry").filter(
            has_text=re.compile(r"\bEducation History\b", re.I)
        ).first
    if not group.count():
        return
    ...
    _choose_education_option(
        page, start, str(education.get("start_month", "")), control_index=0
    )
    _choose_education_option(
        page, start, str(education.get("start_year", "")), control_index=1
    )
    _choose_education_option(
        page, end, str(education.get("end_month", "")), control_index=0
    )
    _choose_education_option(
        page, end, str(education.get("end_year", "")), control_index=1
    )

    student_label = group.locator("label").filter(
        has_text=re.compile(r"Still Student", re.I)
    ).first
    ...
    for field in fields:
        try:
            ...
            if label.count():
                ...
                is_required = "required" in label_class
                if not is_required:
                    pseudo_content = label.evaluate(
                        "el => getComputedStyle(el, '::after').content || ''"
                    )
                    is_required = "*" in str(pseudo_content)
            if not is_required:
                is_required = field.locator(
            ...
            if yes_no_buttons.count() >= 2:
                ...
                for index in range(yes_no_buttons.count()):
                    ...
                    if (
                        ...
                        or re.search(r"(?:^|[_\-\s])(active|selected)(?:[_\-\s]|$)", classes)
                    ):
                        selected = True
                        break
                if not selected:
            ...
            if controls.count() and not any(
                controls.nth(index).input_value().strip()
...
def _capture_verified_submission(
    ...
    screenshot_path = directory / (
        f"{_safe_filename_part(company)}_submitted_verified.png"
    )
    try:
        cdp = page.context.new_cdp_session(page)
        data = cdp.send(
...
def _submit_application(
    ...
    for submit_attempt in range(1, MAX_SUBMIT_ATTEMPTS + 1):
        ...
        try:
            submit = _locate_submit_btn(page)
            if (
                (not submit.count() or not submit.is_visible())
                and _submission_confirmed(_page_body_lower(page))
            ):
        ...
        if _submission_failed(body_text):
            logger.warning(
                "Submission flagged as possible spam by Ashby. "
                "Waiting 5s before retrying..."
            )
            time.sleep(5.0)
            continue
        if (
            _submission_confirmed(body_text)
            and not _locate_submit_btn(page).count()
        ):
            confirmation_found = True
            break
...
def run_job(
    ...
    if isinstance(eeo_defaults, Mapping):
        ...
        profile["veteran"] = (
            eeo_defaults.get("veteran_status") or profile.get("veteran")
        )
        profile["disability"] = (
            eeo_defaults.get("disability_status") or profile.get("disability")
        )
    _merge_repository_rules(profile, defaults, resolved_config)
    missing_identity = [
        field for field in ("first_name", "last_name") if not profile.get(field)
    ]
    if missing_identity:
        raise ValueError(
            "Config missing required candidate fields: "
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            apply = page.locator('a:has-text("Apply for this Job"), button:has-text("Apply for this Job"), a:has-text("Apply"), button:has-text("Apply")').first
            ...
            critical.update(refreshed_personal)
            ...
            missing = [k for k, ok in critical.items() if not ok and k in ("name", "email", "resume")]
            if missing:
            ...
            if required_issues:
                ...
                raise RuntimeError(
                    "Aborting completion: required fields remain unanswered"
                )
