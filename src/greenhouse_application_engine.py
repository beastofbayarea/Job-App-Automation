...
def _upload_resume(page: Page, resume: Path) -> bool:
    ...
    for index in range(inputs.count()):
        ...
        try:
            ...
            if "resume" not in context and "resume" not in (
...
def _open_application_form(
    page: Page, timeout: int, source_url: str, company: str
) -> None:
    """Follow a custom career site's Apply CTA before filling Greenhouse fields."""
    if _first_visible(
        page.locator(
            'input[name="first_name"], input[type="email"], input[type="file"]'
        )
    ) is not None:
        return
    ...
    navigate_reusing_tab(
        page, embed_url, wait_until="domcontentloaded", timeout=timeout
    )
    try:
        page.wait_for_load_state("networkidle", timeout=timeout)
    except PlaywrightTimeoutError:
...
def _select_native(page: Page, label_pattern: str, preferred: Sequence[str]) -> bool:
    try:
        target = _first_visible(
            page.get_by_label(re.compile(label_pattern, re.IGNORECASE))
        )
    except Exception:
        target = None
    if target is None:
                    ...
                    if listbox.count():
                        ...
                        for index in range(exact_text.count()):
                            ...
                            if option.is_visible():
                                option.click()
                                return True
                        text_matches = listbox.get_by_text(
                            re.compile(re.escape(desired), re.I)
                        )
...
def _fill_standard_fields(
    ...
    fields = {
        ...
        "location": False,
        "linkedin": _fill_labeled(
            page, r"linkedin", str(profile.get("linkedin", ""))
        ),
        "website": _fill_labeled(
            page, r"(?:website|portfolio)", str(profile.get("website", ""))
        ),
    ...
    for index in range(controls.count()):
        ...
        try:
            ...
            if control_type in {"checkbox", "radio"}:
                ...
                missing.append(label or control.get_attribute("name") or control.get_attribute("id") or f"field-{index}")
        except Exception:
            continue
